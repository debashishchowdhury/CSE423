...
def init_traps():
    global traps
    traps = []
    # Place traps strategically in corridor areas (hall is centered at 0,0)
    for i in range(3):
        # Random corridor positions
        x = random.uniform(-HALL_HALF + 20, HALL_HALF - 20)  # Stay in corridor
        y = random.uniform(-HALL_LEN * 0.4, HALL_LEN * 0.4)  # Along corridor length
...
def update_monster(dt):
    global mx, my, m_visible, m_visible_timer, m_speed
    if not m_visible:
        return
    ...
    m_speed = m_speed_base + 40.0 * collected_clues()
    if scooby_active:
...
def update_player(dt):
    ...
    # WASD for pure movement only
    if b'w' in keys_down: move += 1.0
    if b's' in keys_down: move -= 1.0
    if b'a' in keys_down: strafe -= 1.0  # Strafe left (not rotate)
    if b'd' in keys_down: strafe += 1.0  # Strafe right (not rotate)

    # No camera rotation from WASD - only from arrow keys
    # (Arrow key rotation is handled in special_down function)

    pdir += rot
    # forward vector from pdir
    rad = math.radians(pdir)
    fy = math.cos(rad); fx = math.sin(rad)
    nx, ny = -fy, fx  # right-hand perpendicular for strafe
    speed = pspd * (1.2 if scooby_active else 1.0)
    ...
    if -HALL_HALF - 50 <= nx <= HALL_HALF + 50:  # Expanded corridor area for door access
        ...
        if abs(nx) <= HALL_HALF:
            ...
            for room_y in ROOM_Y:
                ...
                if (nx < -HALL_HALF and
                    room_y - door_half <= ny <= room_y + door_half):
                    # Allow access to left room through door
                    return nx, ny

                # Check if trying to access right room door
                if (nx > HALL_HALF and
                    room_y - door_half <= ny <= room_y + door_half):
                    # Allow access to right room through door
                    return nx, ny
...
def update_camera():
    ...
    if camera_mode == CAM_TPS:
        ...
        cx = px - math.sin(rad) * TPS_DIST
        cy = py - math.cos(rad) * TPS_DIST
        cz = TPS_HEIGHT
        ...
        tx, ty, tz = px + math.sin(rad)*40.0, py + math.cos(rad)*40.0, 65.0
        gluLookAt(px, py, 65.0, tx, ty, tz, 0, 0, 1)
...
def draw_hud():
    ...
    if is_player_in_room():
        ...
        draw_text(x0, y0 - 56, "STATUS: SAFE IN ROOM")
        glColor3f(1.0, 1.0, 1.0)  # Reset to white
    else:
        glColor3f(1.0, 0.3, 0.3)  # Red for danger
        draw_text(x0, y0 - 56, "STATUS: CORRIDOR (DANGER)")
        glColor3f(1.0, 1.0, 1.0)  # Reset to white

    # Boost status
    boost_str = "Ready" if boost_available() else ("Active" if scooby_active else f"CD: {scooby_cd_left:0.1f}s")
    draw_text(x0, y0 - 84, f"Boost: {boost_str}")

    # Instructions
    glColor3f(0.8, 0.8, 0.8)  # Gray for instructions
    draw_text(x0, y0 - 112, "WASD: Move  Arrows: Camera  1: TPS  2: FPS  U: Overhead  B: Boost  R: Restart")
    draw_text(x0, y0 - 140, "Enter rooms through doors for safety!")
    ...
    if unlocked_achievements:
        ...
        draw_text(x0, y0 - 168, f"Achievements: {len(unlocked_achievements)}/7")
        ...
        for i, achievement_name in enumerate(unlocked_achievements[-3:]):
            ...
            draw_text(x0, y0 - 196 - i*20, f"✓ {achievement['name']}")
        glColor3f(1.0, 1.0, 1.0)  # Reset to white
...
def keyboard_down(k, x, y):
    ...
    if k == b'u':
        camera_mode = third_person_mode  # activate overhead
        third_person_height = 520.0      # set to show whole board
        third_person_angle = 0.0         # reset angle
    if k == b'r': reset_game()
...
def display():
    ...
    if monster_dist < 100:  # Monster nearby - flickering, eerie lighting
        flicker = 0.7 + 0.3 * math.sin(t * 8.0) * math.sin(t * 13.0)
        glLightfv(GL_LIGHT0, GL_DIFFUSE, (0.8 * flicker, 0.4 * flicker, 0.6 * flicker, 1.0))
        glLightfv(GL_LIGHT0, GL_AMBIENT, (0.2, 0.1, 0.2, 1.0))  # Darker ambient
    elif clue_count >= 3:  # Many clues - bright revelation lighting
        brightness = 1.4 + 0.2 * math.sin(t * 2.0)
        glLightfv(GL_LIGHT0, GL_DIFFUSE, (brightness, brightness, 0.9 * brightness, 1.0))
        glLightfv(GL_LIGHT0, GL_AMBIENT, (0.5, 0.5, 0.4, 1.0))  # Bright ambient
    elif is_player_in_room():  # Safe in room - warm, steady lighting
        glLightfv(GL_LIGHT0, GL_DIFFUSE, (1.2, 1.0, 0.8, 1.0))  # Warm white
        glLightfv(GL_LIGHT0, GL_AMBIENT, (0.4, 0.35, 0.3, 1.0))  # Warm ambient
    else:  # Normal corridor - cool, dim lighting
        pulse = 0.9 + 0.1 * math.sin(t * 1.5)
        glLightfv(GL_LIGHT0, GL_DIFFUSE, (0.7 * pulse, 0.8 * pulse, 1.0 * pulse, 1.0))
        glLightfv(GL_LIGHT0, GL_AMBIENT, (0.3, 0.3, 0.4, 1.0))  # Cool ambient

    # Light follows player but with some atmospheric offset
    light_offset_x = 20 * math.sin(t * 0.5)  # Gentle swaying
    light_offset_y = 10 * math.cos(t * 0.7)
    glLightfv(GL_LIGHT0, GL_POSITION, (px + light_offset_x, py - 120.0 + light_offset_y, 160.0, 1.0))
